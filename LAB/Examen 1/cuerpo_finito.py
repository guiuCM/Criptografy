class G_F:
    ...
    def generar_tablas(self):
        ...
        Tabla_EXP = [0] * self.Tamaño_Cuerpo    #0-255
        Tabla_LOG = [0] * self.Tamaño_Cuerpo    #0-255

        # Inicializar valores iniciales
        valor = 1
        for i in range(self.Tamaño_Cuerpo - 1):
            Tabla_EXP[i] = valor
            Tabla_LOG[valor] = i
            valor = (valor * 2) % self.Tamaño_Cuerpo
            if valor >= self.Tamaño_Cuerpo:
                valor ^= self.Polinomio_Irreducible  #mod (0x11B)

        # La última entrada de Tabla_EXP
        Tabla_EXP[self.Tamaño_Cuerpo - 1] = Tabla_EXP[0]
    ...
    # que es el producto en el cuerpo de 'n' y 0x02 (el polinomio X).
        ...
        if resultado >= self.Tamaño_Cuerpo:
...
#prova
cf = G_F()
n = 0x10  # Ejemplo de un valor en el cuerpo finito
numero = 0x2A
numero2 = 0x3B

resultado = cf.xTimes(n)
result = cf.producto(numero,numero2)
result2 = cf.inverso(n)
